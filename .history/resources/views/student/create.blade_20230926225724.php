...
<form action="{{ route('student.store') }}" method="POST" class="form">
    @csrf
        ...
        <table border="1" class="table">
            <label for="admision_number">Admision Number</label><input type="text" name="admision_number" id=""class="form-control" value="{{old()}}"><br>
            @if($errors->has('admision_number'))
             ($errors->has('admision_number'))
            <span>{{ $errors->first('admission_number') }}</span>
            @endif

            <label for="first_name">first name</label><input type="text" name="first_name" id=""class="form-control" value="{{old()}}"><br>
            @if($errors->has('first_name'))
            <span>{{ $errors->first('first_name') }}</span>
            @endif

            <label for="last_name">last name</label><input type="text" name="last_name" id="" class="form-control" value="{{old()}}"><br>
        ...
        @if($errors->has('gender'))
            ...
            <label for="address">Address</label><input type="text" name="address" id="" class="form-control" value="{{old()}}"><br>
         @if($errors->has('address'))
            <span >{{ $errors->first('address') }}</span>
            @endif

            <label for="dob">dob</label><input type="date" name="dob" id="" class="form-control" value="{{old()}}"><br>
     @if($errors->has('dob'))
            <span >{{ $errors->first('dob') }}</span>
            @endif

        <label for="house_id">House</label> &nbsp; <button type="button" class="badge rounded-pill bg-info"><a class="link-light" href="{{ route('house.create') }}" value="{{old()}}"> Create House</a></button> <br>
      @if($errors->has('house_id'))
            <span >{{ $errors->first('house_id') }}</span>
            @endif
            ...
            <label for="nic">NIC</label><input type="text" name="nic" id="" class="form-control" value="{{old()}}"><br>
            @if($errors->has('nic'))
