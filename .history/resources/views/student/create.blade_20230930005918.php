...
<form action="{{ route('student.store') }}" method="POST" class="form">
    ...
    <center>
        ...
        <i class="fa-solid fa-shield-halved fa-sm">
            <label for="admision_number">Admision Number</label>
            <input type="text" name="admision_number" id=""class="form-control"><br>
            @if($errors->has('admision_number'))
            <div class="alert alert-danger d-flex align-items-center" role="alert">
                 {{ $errors->first('admision_number') }}</i></div>
