...
<table class="table table-light table-striped" border="1">
    ...
    <tr>
        ...
        <form action="{{ route('subject.destroy',$subject) }}" method="POST" class="form">
            @csrf
            @method('delete')
        <td><button type="submit" class="btn btn-info"><a class="link-light" href="{{ route('subject.show',$subject) }}">show</button></a<button type="submit" class="btn btn-warning"><a class="link-light" href="{{ route('subject.edit',$subject) }}">edit</button></a>
